// ════════════════════════════════════════════════════════
//  DORM-OS v1.2 — MIT ADT Pune
//  Hardware: NodeMCU ESP8266 + SSD1306 OLED 0.96"
//           + DHT11 + Red LED (D6/GPIO12) + Buzzer (D7/GPIO13)
//
//  Changes from v1.0:
//   • Telegram send/receive → blinks RED LED instead of beeping
//   • Power cuts now counted via RTC memory (survives reboot, not WiFi drops)
//   • AI backend switched from Gemini to Grok (xAI)
//
//  Changes from v1.1:
//   • Internet lost → 3 buzzer beeps + RED LED blinks for 60 s
// ════════════════════════════════════════════════════════
...
#define BUZZER    13   // D7 — active-HIGH, idle LOW (only used for alarm now)
...
// ── Grok (xAI) API ───────────────────────────────────────
// Regenerate your key at: console.x.ai  ← DO THIS NOW if key was shared publicly
...
// ════════════════════════════════════════════════════════
//  POWER CUT COUNTER — stored in RTC memory (survives reset/power loss)
//  RTC memory slot 65 = magic number, slot 66 = count
// ════════════════════════════════════════════════════════
...
// ════════════════════════════════════════════════════════
//  BUZZER — ALARM ONLY (active-HIGH, idle LOW)
// ════════════════════════════════════════════════════════
void alarmBeep(int count, int onMs = 150, int offMs = 80) {
  // Only used for the actual wake-up alarm, NOT for Telegram activity
...
// ── Telegram LED flash (non-blocking, 2 quick blinks) ───
void tgLedFlash() {
  // Quick synchronous double-blink to signal Telegram activity
  // Short enough (160ms total) not to block anything critical
  for (int i = 0; i < 2; i++) {
...
// ════════════════════════════════════════════════════════
//  TELEGRAM SEND  ← LED blink instead of beep
// ════════════════════════════════════════════════════════
void tgSend(const String& msg) {
  ...
  if (ok) {
    tgLedFlash();          // ← 2 quick blinks = message sent OK
  } else {
    startBlink(RED_LED, 3, 200);  // 3 rapid blinks = send failed
  }
  Serial.println(ok ? "[TG] OK: " + msg : "[TG] FAIL");
}

// ════════════════════════════════════════════════════════
//  GROK AI  (replaces Gemini)
// ════════════════════════════════════════════════════════
String askGrok(String userMsg) {
  ...
  // Build sensor context as the system instruction
  ...
  systemText += "Current sensor data — ";
  ...
  if (out.ready) {
    systemText += "Outside: " + String(out.temp, 1) + "°C, ";
    systemText += out.condition + ", ";
    systemText += "Rain: " + String(out.rainChance) + "%, ";
    systemText += "UV: " + String(out.uvMax, 0) + ", ";
  ...
  // ── xAI /v1/responses payload ─────────────────────────
  // Format: { model, instructions (system), input (user), max_output_tokens }
  String payload = "{";
  payload += "\"model\":\"grok-3-mini\",";          // fast model — good for ESP8266
  // If you want reasoning: \"grok-4.20-reasoning\" but increase setTimeout to 40000
  payload += "\"max_output_tokens\":150,";
  ...
  String url = "https://";
  url += GROK_HOST;
  url += "/v1/responses";           // ← correct xAI endpoint

  https.begin(client, url);
  https.addHeader("Content-Type",  "application/json");
  https.addHeader("Authorization", "Bearer " + String(GROK_API_KEY));
  https.setTimeout(25000);          // 25s — reasoning models can be slow
  ...
  if (code == 200) {
    ...
    Serial.println("[Grok] Raw: " + response.substring(0, 200)); // debug first 200 chars

    // /v1/responses returns: { "output": [ { "type":"message", "content": [ { "text":"..." } ] } ] }
    DynamicJsonDocument doc(4096);
    DeserializationError err = deserializeJson(doc, response);
    if (!err) {
      // Walk output array to find first message text
      JsonArray outputArr = doc["output"].as<JsonArray>();
      for (JsonObject item : outputArr) {
      ...
      Serial.println("[Grok] JSON parse error: " + String(err.c_str()));
    }
  } else {
    Serial.printf("[Grok] HTTP %d\n", code);
    Serial.println(https.getString().substring(0, 200));
  }
  https.end();
  return reply;
}
...
// ════════════════════════════════════════════════════════
//  WIFI RECONNECT
//  powerCuts increments here because a WiFi reconnect after
//  a full power outage is different from a normal WiFi drop.
//  We only count it as a power cut when the ESP reboots
//  (detected via RTC memory magic number surviving reboot).
// ════════════════════════════════════════════════════════
bool          wifiWasUp       = true;
  ...
  if (WiFi.status() != WL_CONNECTED) {
    if (wifiWasUp) {
      ...
      if (!alertActive[A_WIFI_LOST]) {
        alarmBeep(3, 200, 150);              // 3 beeps on internet lost
        startBlink(RED_LED, 50, 600);        // blink for ~60 s (50 pairs × 1200 ms)
    ...
    if (!wifiWasUp) {
      wifiWasUp = true;
      stopBlink();
      // WiFi dropped and came back — NOT counted as power cut
      // (power cuts are counted at boot via RTC memory)
      if (alertActive[A_WIFI_LOST]) {
...
void sendDailyReport() {
  ...
  if (h == 23 && !reportSentToday) {
    ...
    // Reset daily stats + power cut counter
    room.maxTemp  = -99;
    room.minTemp  = 99;
    room.sumTemp  = 0;
    room.readings = 0;
    resetPowerCuts();   // ← reset RTC counter at end of each day
  }
...
void checkAlarm() {
  ...
  if (alarm.ringing) {
    // Buzz every 5 seconds — BUZZER is OK here, this is the actual alarm
...
void checkFocusTimer() {
  ...
  if (elapsed >= (unsigned long)focusMins) {
    focusMode = false;
    stopBlink();
    alarmBeep(3, 200, 100);   // Focus done = buzzer is fine here too
...
void handleCommand(String text) {
  ...
  tgLedFlash();   // ← Blink LED when command received (replaces beep)
...
void startup() {
  ...
  display.setCursor(4, 34);  display.print("V1.2  by JARVIS");
...
void connectWiFi() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    // Startup chime still uses buzzer (one-time, not chat related)
...
void setup() {
  ...
  // ── Detect power cut via RTC memory ─────────────────
  // If RTC magic is valid, this is a soft reboot / power cut reboot
  // If not valid, this is first-ever boot (flash new)
  loadPowerCuts();
  uint32_t prevMagic = 0;
  ESP.rtcUserMemoryRead(RTC_SLOT_MAGIC, &prevMagic, sizeof(prevMagic));
  if (prevMagic == RTC_MAGIC) {
    // Magic survived → board was previously running → this boot = power cut
    incrementPowerCuts();
    Serial.printf("[Boot] Power cut detected! Total today: %d\n", rtcPowerCuts);
  } else {
    Serial.println("[Boot] First boot, initialising RTC counter.");
  }
